...
describe('App workspace state', () => {
  ...
  it('opens an agent-created document after its registry is synchronized', async () => {
    render(<App />)
    sendWorkspaceInfo({ type: 'workspaceInfo', path: '/tmp/agent-document', name: 'agent-document', available: true })

    sendWorkspaceInfo({
      type: 'loadDocTypes',
      mode: 'replace',
      data: [{ id: 'doc-ux-plan', name: 'UX Plan', icon: 'article', createdAt: 1, order: 0 }],
    })
    sendWorkspaceInfo({ type: 'navigateTo', view: { page: 'doc-ux-plan' } })
